...
float distancePow2(__global const float2 *p1, __global const float2 *p2) {
    return pow(p1->x - p2->x, 2) + pow(p1->y - p2->y, 2);
}
float distance(__global const float2 *p1, __global const float2 *p2) {
    return sqrt(distancePow2(p1, p2));
}

float2 calculate_force_gpu(__global float2 *p1, __global float2 *p2, float mass){
    float2 force_vector;
